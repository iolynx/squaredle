    ...
    if (isDragging) {
      setSelectedPath((prevPath) => {
        ...
        if (Math.abs(lastPos[0] - rowIndex) <= 1 && Math.abs(lastPos[1] - colIndex) <= 1) {
          return [...prevPath, [rowIndex, colIndex]];
        }
        return prevPath;
      });
    }
  ...
  const renderPath = () => {
    if (selectedPath.length > 1) {
      ...
      return selectedPath.map((pos, index) => (
        <line
          key={index}
          x1={pos[1] * 100 + 120} // Adjust for the position in the grid
          y1={pos[0] * 100 + 120}
          x2={(selectedPath[index + 1] ? selectedPath[index + 1][1] : pos[1]) * 100 + 120}
          y2={(selectedPath[index + 1] ? selectedPath[index + 1][0] : pos[0]) * 100 + 200}
          stroke="red"
          strokeWidth="5"
        />
      ));
          ...
          {board.map((row, rowIndex) =>
            row.map((char, colIndex) => (
              <Tile
                ...
                onMouseMove={(e) => onMouseMove(rowIndex, colIndex, e)}
                onMouseUp={onMouseUp}
              >
                {char}
              </Tile>
